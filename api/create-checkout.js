...
// Starts a Stripe Checkout for the one-time $19.99 Earth Quest purchase.
// No login required — Checkout collects the buyer's email, which the
// webhook uses to send them their private dashboard link.
// =====================================================================
import { stripe, PRICE_EARTHQUEST, SITE_ORIGIN, adminDb, corsHeaders, json } from './_shared.js';
...
export async function POST(request) {
  ...
  try {
    ...
    const metadata = { product: 'earth_quest' };
    ...
    if (token) {
      const { data: owner } = await adminDb().from('eq_owners')
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      line_items: [{ price: PRICE_EARTHQUEST, quantity: 1 }],
