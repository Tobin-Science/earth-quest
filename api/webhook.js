...
// When an Earth Quest purchase completes, create the owner (with a secret
...
export async function POST(request) {
  ...
  const isQuest = s.mode === 'payment' && s.metadata && s.metadata.product === 'earth_quest';
  if (!isQuest) return new Response('not an earth quest purchase', { status: 200 });
